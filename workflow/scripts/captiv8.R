...
# Modified 2025-04-24 for external use by Paul Gordon, gordonp@ucalgary.ca (PR2C)
...
if(!require(plyr)){install.packages("plyr", repos = CRAN_mirror)}
if(!require(dplyr)){install.packages("dplyr", repos = CRAN_mirror)}
if(!require(optparse)){install.packages("optparse", repos = CRAN_mirror)}
if(!require(ggplot2)){install.packages("ggplot2", repos = CRAN_mirror)}
if(!require(devtools)){install.packages("devtools", repos = CRAN_mirror)}
if(!require(BiocManager)){install.packages("BiocManager", repos = CRAN_mirror)}
if(!require(biomaRt)){BiocManager::install("biomaRt", dependencies=TRUE)}
if(!require(CMSclassifier)){install_github("Sage-Bionetworks/CMSclassifier", dependencies=TRUE)}
...
option_list = list(
    make_option(c("-m", "--meta"), type="character", default=NULL, 
              help="POG case", metavar="character")
); 
...
meta_file <- read.delim(opt$meta, header=FALSE, sep='\t')
...
m1m2_exp <- subset(exp, exp$gene_id %in% M1M2_genes)
...
if (colon %in% c("yes", "Yes", "YES", "y", "Y")){
  ...
  matching_ids <- getBM(attributes=c('entrezgene_id', 'hgnc_symbol', 'ensembl_gene_id'), 
      filters = 'ensembl_gene_id', 
      values = exp$gene_id, 
      mart = ensembl)

  exp <- merge(exp, matching_ids, by.x="gene_id", by.y="ensembl_gene_id", all.x=TRUE)
...
cms_score <- case_when(colon == "yes" && cms_res$SSP.nearestCMS == "CMS1" ~ 3, 
                       TRUE ~ 0)
