...
mkdir -p results/analysis/primary/HiSeq
...
# Remove the SRA archive directories now that we have the FASTQ files extracted
rm -rf SRR6702602 SRR6702601

# There is no "normal" sample here, only tumor. 
# So generate a pseudo-normal input, by removing most reads with somatic mutations from the FASTQ files, using a k-mer table of 30 million common nucleotide 
...
# Only include read pairs where both reads appear to not have any somatic mutations.
