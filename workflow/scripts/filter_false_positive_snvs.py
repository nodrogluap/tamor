...
# Default SNV hadd filtering for germline samples is really low (phred value of 3). For FFPE or degraded samples especially, this leads
# to many false positive SNVs. If configured, use the new threshold to change PASS to filtered.
min_qual = 0
if "min_site_variant_phred_threshold" in config:
    min_qual = config["min_site_variant_phred_threshold"]
...
total_low_qual_site_filtered = 0
total_low_qual_site_filter_added = 0
total_FP_filtered = 0
...
with open(tmpfile.name, 'wt') as new_vcf:
    ...
    with gzip.open(args.vcf, 'rt') as f:
        for line_num, line in enumerate(f, start=1):
            if in_header:
                if line.startswith("##FILTER=<ID=posthoc_low_qual_site_filtering"):
                    continue
                ...
                if line.startswith("##referenceSexKaryotype="): # This line occurs in Dragen right after the FILTER description header lines
                    ...
                    if(min_qual > 0):
                        print (f'##FILTER=<ID=posthoc_low_qual_site_filtering,Description="Variants in this position fail the configured Tamor variant site threshold of {min_qual} or site+mapping qualities < {min_qual+50}">', file=new_vcf)
                    if(filter_Alus):
            ...
            if len(fields) < 10 or len(fields) > 11: 
                raise Exception('Expected 10 (germline) or 11 (somatic) tab-delimited columns but found ' + str(len(fields)) + " at "+args.vcf+":"+str(line_num))
            ...
            if "posthoc_low_qual_site_filtering" in fields[6]:
                fields[6] = fields[6].replace("posthoc_low_qual_site_filtering;","")
                fields[6] = fields[6].replace(";posthoc_low_qual_site_filtering","")
                fields[6] = fields[6].replace("posthoc_low_qual_site_filtering","")
            ...
            # Do the low site quality check.
            if min_qual > 0:
                # In germline chrM and maybe a few other rare instances might not have a qual.
                if fields[5] == ".":
                    qual = 10000 # exclude it from filtering
                else:
                    qual = float(fields[5])
                # Low site qual for sure.
                if qual < min_qual:
                    total_low_qual_site_filtered = total_low_qual_site_filtered + 1
                    if fields[6] == "PASS":
                        total_low_qual_site_filter_added = total_low_qual_site_filter_added + 1
                        fields[6] = "posthoc_low_qual_site_filtering"
                    else:
                        fields[6] = ";".join(["posthoc_low_qual_site_filtering"])
                # Might still need to be filtered based on combined site + mapping quals.
                elif qual < 50 + min_qual:
                    match = re.search("MQ=(\\d+)", fields[7]) # search the INFO column for the mapping quality
                    if match and float(match.group(1))+qual < 50 + min_qual:
                        if fields[6] == "PASS":
                            total_low_qual_site_filter_added = total_low_qual_site_filter_added + 1
                            fields[6] = "posthoc_low_qual_site_filtering"
                        else:
...
with open(args.output_metrics, "wt") as metrics:
        ...
        print("VARIANT CALLER POSTFILTER CUSTOM,,Variant filter with posthoc_low_qual_site_filtering,"+str(total_low_qual_site_filter_added)+","+str(total_low_qual_site_filter_added/total_orig), file=metrics)
        print("VARIANT CALLER POSTFILTER CUSTOM,,Variant filter changed PASS to posthoc_low_qual_site_filtering,"+str(total_low_qual_site_filtered)+","+str(total_low_qual_site_filtered/total_orig_pass), file=metrics)
