...
system(f"tail -n +2 {tumor_expr_tpm_tsv} | perl -ane 'BEGIN{{%blacklist = split /\\s/s, `cat config/transcript_id_comparison_blacklist.tsv`}}$F[0] =~ s/\\.\\d+$//; print \"$F[0]\\t$F[3]\\n\" unless exists $blacklist{{$F[0]}}' >> {TPMFILE}")
