...
resource_dict = {
        "internal_pcgr": ["http://insilico.hpc.uio.no/pcgr/pcgr_ref_data.20240927.grch38.tgz", "data", "tar", "xzf"],
...
for resource_name, spec in resource_dict.items():
    if(not os.path.exists(spec[1])):
        ...
        if result.returncode != 0:
            raise SystemExit("FATAL: Aborting reference data download, received non-zero return code ("+result.returncode+") from command: " +
        ...
        if result.returncode != 0:
            raise SystemExit("FATAL: Received non-zero return code ("+result.returncode+") from command: mv " + local_path+".part" + local_path)
        ...
        if len(spec) >= 3:
            ...
            if result.returncode != 0:
                raise SystemExit("FATAL: Aborting reference data download, received non-zero return code ("+result.returncode+") from command: " + post_process_cmd)

            print("INFO: Removing source file after post-processing: "+ local_path)
            result = subprocess.run(["rm", local_path])
            if result.returncode != 0:
                print("WARNING: Received non-zero return code ("+result.returncode+") from source cleanup command: rm " + local_path)
...
if result.returncode != 0:
    raise SystemExit("FATAL: Received non-zero return code (" + result.returncode + ") from command: " + nirvana_downloader_path + " --ga GRCh38 --out nirvana")
