...
rule dragen_germline_snv_sv_and_cnv_calls:
        ...
        output:
                ...
                (config["output_dir"]+'/{project}/{subject}/{subject}_{normal}.dna.germline.cram' if config["generate_crams"] else config["output_dir"]+'/{project}/{subject}/{subject}_{normal}.dna.germline.bam'),
                config["output_dir"]+'/{project}/{subject}/{subject}_{normal}.dna.germline.microsat_normal.dist',
                config["output_dir"]+'/{project}/{subject}/{subject}_{normal}.dna.germline.mapping_metrics.csv',
                config["output_dir"]+'/{project}/{subject}/{subject}_{normal}.dna.germline.wgs_coverage_metrics.csv',
        ...
        run:
                ...
                if has_UMIs:
                        ...
                        if(not os.path.exists(normal_bam)):
                                dragen_cmd_align = "dragen -r "+config["ref_genome"]+" --ora-reference "+config["ref_ora"]+" --enable-map-align true --enable-map-align-output true --enable-bam-indexing true --fastq-list {this_sample_only_fastq_list_csv} --fastq-list-all-samples true --output-directory "+ config["output_dir"]+"/{wildcards.project}/{wildcards.subject} --output-file-prefix {wildcards.subject}_{wildcards.normal}.dna.germline --enable-hla true --intermediate-results-dir "+ config["temp_dir"]+" -f --umi-enable true {umi_correction_flag} --umi-min-supporting-reads 1 --umi-min-map-quality 1 --enable-down-sampler true --down-sampler-coverage 60 --soft-read-trimmers polyg --read-trimmers adapter --trim-adapter-read1 resources/adapter_sequences/read1_3prime.fasta --trim-adapter-read2 resources/adapter_sequences/read2_3prime.fasta"
                                
                                if is_dragen_v42:
                                        dragen_cmd_align = dragen_cmd_align + " --hla-enable-class-2 true"
                                if config["generate_crams"]:
...
rule dragen_somatic_snv_sv_and_cnv_calls:
        ...
        run:
                ...
                if germline_has_UMIs or tumor_has_UMIs:
                        ...
                        dragen_cmd_align = "dragen -r "+config["ref_genome"]+" --ora-reference "+config["ref_ora"]+" --enable-map-align true --enable-map-align-output true --enable-bam-indexing true --tumor-fastq-list {this_sample_tumor_only_fastq_list_csv} --tumor-fastq-list-all-samples true --output-directory "+ config["output_dir"]+"/{wildcards.project}/{wildcards.subject} --output-file-prefix {wildcards.subject}_{wildcards.tumor}_{wildcards.normal}.dna.somatic --enable-hla true --intermediate-results-dir "+config["temp_dir"]+" -f --umi-enable true {umi_correction_flag} --umi-min-supporting-reads 1 --umi-min-map-quality 1 --soft-read-trimmers polyg --read-trimmers adapter --trim-adapter-read1 resources/adapter_sequences/read1_3prime.fasta --trim-adapter-read2 resources/adapter_sequences/read2_3prime.fasta"
                        if config["generate_crams"]:
                                dragen_cmd_align = dragen_cmd_align + " --output-format CRAM"
                        if config["ora_compress_fastqs"]:
                                dragen_cmd_align = dragen_cmd_align + " --enable-ora true --ora-delete-input-files true"
                        ...
                        dragen_cmd = "dragen -r "+config["ref_genome"]+" --enable-map-align false "+get_aligned_input_param(input.germline_bam)+" --tumor-bam-input {tumor_bam} --output-directory {output_temp_dir} --output-file-prefix {wildcards.subject}_{wildcards.tumor}_{wildcards.normal}.dna.somatic --intermediate-results-dir "+config["temp_dir"]+" -f"+" --enable-variant-caller true --enable-cnv true --cnv-use-somatic-vc-baf true --cnv-normal-cnv-vcf {params.germline_cnv} --enable-sv true --vc-enable-unequal-ntd-errors=true --vc-enable-trimer-context=true --vc-enable-umi-solid true --msi-command tumor-normal --msi-coverage-threshold " + str(config["msi_min_coverage"]) + " --msi-microsatellites-file {input.msi_sites} --enable-hrd true --enable-variant-annotation=true --variant-annotation-data=resources/nirvana --variant-annotation-assembly=GRCh38 --enable-tmb true"
                        if is_dragen_v42:
